...
export default function Page() {
  ...
  const [columns, setColumns] = useState<Record<string, any>>({
    ...
    name: { label: "Dibuat oleh", visible: true },
    date: { label: "Tanggal transaksi", visible: true },
    jumlahTransaksi: { label: "Jumlah", visible: true },
  });
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        includesText(r.name.toLowerCase(), q) ||
        includesText(r.namaCabang, q) ||
        includesText(r.noInvoice, q) ||
        includesText(r.date, q)
    );
  }, [rows, filterText]);
  ...
  const renderTableBody = () => {
    ...
    return filteredRows.map((r) => (
      <tr key={r._id} className="border-b border-gray-100">
        {columns.namaCabang.visible && (
          <td className="px-5 py-3 text-sm text-gray-900">
            {r.branch?.name || "-"}
          </td>
        )}
        {columns.noInvoice.visible && (
          <td className="px-5 py-3 text-sm text-gray-900">
            {r.noInvoice.toUpperCase() || "-"}
          </td>
        )}
        ...
        {columns.date.visible && (
          <td className="px-5 py-3 text-sm text-gray-700">{r.date}</td>
        )}
        {columns.jumlahTransaksi.visible && (
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const result = await fetch(`/api/transaction?id=${user._id}`, {
        method: "GET",
        headers: {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div className="text-xs text-gray-400">Laporan Transaksi</div>
          </div>
        </div>
      </div>
