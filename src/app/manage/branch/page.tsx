...
export default function Page() {
  ...
  const [columns, setColumns] = useState<Record<string, any>>({
    name: { label: "Nama Cabang", visible: true },
    noTeleponAdmin: { label: "No Telepon Admin", visible: true },
    alamat: { label: "Lokasi Cabang", visible: true },
  });
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        ...
        includesText(r.noTeleponAdmin, q) ||
        includesText(r.alamat, q)
    );
  }, [rows, filterText]);
  ...
  const actionRows = (id: string) => {
    return (
      <ActionMenu
        ...
        items={[
          { label: "Edit", onClick: () => router.push(`/manage/branch/${id}/edit`) },
  ...
  const renderTableBody = () => {
    ...
    return filteredRows.map((r) => (
      <tr key={r._id} className="border-b border-gray-100">
        ...
        {columns.noTeleponAdmin.visible && (
          <td className="px-5 py-3 text-sm text-gray-700">
            {r.noTeleponAdmin || "-"}
          </td>
        )}
        {columns.alamat.visible && (
  ...
  const handleExport = () => {
    ...
    const data = filteredRows.map((r) =>
      visibleCols.map(([key]) => {
        ...
        if (key === "noTeleponAdmin") return r.noTeleponAdmin || "-";
        if (key === "alamat") return r.alamat || "-";
        return "";
      })
    );
    exportToExcel("cabang", headers, data);
  };
  ...
  return (
    <AppShell>
      ...
      <div className="mt-4 flex items-center gap-2">
        <button
          onClick={() => router.push("/manage/branch/add")}
